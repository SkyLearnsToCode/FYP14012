...
static void drawGround()
{
  ...
  */
  ...
  float slope = slope_value*M_PI/180.0;

  glBegin (GL_QUADS);
  glNormal3f (0,0,1);
  glTexCoord2f (-gsize*ground_scale + ground_ofsx,
		-gsize*ground_scale + ground_ofsy);
  glVertex3f (-gsize*cos(slope) + -gsize*sin(slope),
  			gsize*sin(slope) + -gsize*cos(slope),offset);
  glTexCoord2f (gsize*ground_scale + ground_ofsx,
		-gsize*ground_scale + ground_ofsy);
  glVertex3f (gsize*cos(slope) + -gsize*sin(slope),
  			-gsize*sin(slope) + -gsize*cos(slope),offset);
  glTexCoord2f (gsize*ground_scale + ground_ofsx,
		0*ground_scale + ground_ofsy);
  glVertex3f (gsize*cos(slope) + 0*sin(slope),
  			-gsize*sin(slope) + 0*cos(slope),offset);
  glTexCoord2f (-gsize*ground_scale + ground_ofsx,
		0*ground_scale + ground_ofsy);
  glVertex3f (-gsize*cos(slope) + 0*sin(slope),
  			gsize*sin(slope) + 0*cos(slope),offset);
...
/*#else
  if (width >= height) {
    float k2 = float(height)/float(width);
    glOrtho (-vnear*k,vnear*k,-vnear*k*k2,vnear*k*k2,vnear,vfar);
  }
  else {
    float k2 = float(width)/float(height);
    glOrtho (-vnear*k*k2,vnear*k*k2,-vnear*k,vnear*k,vnear,vfar);
  }
#endif*/
...
#ifndef SIM_2D
  // draw the background (ground, sky etc)#ifndef SIM_2D
  drawSky (view2_xyz);
  drawGround();

...
struct Environment {
	...
	Environment( double gx, double gy, double gz ) : space(0), gx(gx), 
													 gy(gy), gz(gz), 
													 contact_cfm(GLB_CFM)	{
	...
	void setSlope( double degree ) {
		slope = degree; 
		ground.setParams(sin(degree*M_PI/180.0), cos(degree*M_PI/180.0), 0 , 0);
		dVector4 result; 
		ground.getParams(result);
		//gx = 0;
		//gy = -gmag*cos(degree*M_PI/180.0);
		//gz = -gmag*sin(degree*M_PI/180.0);
		//world.setGravity(gx, gy, gz);
	}
